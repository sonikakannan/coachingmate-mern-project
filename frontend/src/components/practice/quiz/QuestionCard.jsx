...
const QuestionCard = ({
  ...
  const getOptionStyle = (option) => {
    if (clicked) {
      ...
      if (option === selected && selected !== correctAnswer) return 'bg-red-200 border-red-600';
    }
    ...
    <div className=" p-6  max-w-2xl md:max-w-5xl mx-auto">
      <div className="mb-4">
        <div className="text-sm flex justify-between py-1 text-gray-600 mb-1">
                <Link to={'/practice/quiz'}><FaArrowLeftLong className='text-2xl cursor-pointer '/>
                </Link>
...
<div className="text-xl font-bold  text-gray-800 mb-4">{questionData?.question}</div>
<ul className="space-y-3 ">
  {questionData?.options?.map((option, idx) => (
    <li
      key={idx}
      className={`p-4 rounded border border-indigo-500 text-indigo-700 cursor-pointer text-lg font-medium ${getOptionStyle(option)}`}
