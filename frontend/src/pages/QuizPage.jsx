...
const QuizPage = () => {
  ...
  if (isLoading)
    return <div className="text-center mt-10 text-lg font-medium">Loading quizzes...</div>;

  if (error)
    return <div className="text-center mt-10 text-red-600 text-lg font-medium">Failed to load quizzes.</div>;

  const courses = Array.isArray(data?.courses) ? data.courses : [];

  // Only show courses that have at least one quiz
  const coursesWithQuizzes = courses.filter(course => course.quiz && course.quiz.length > 0);
          ...
          coursesWithQuizzes.map((course, index) => (
            <Link to={`/practice/quiz/quiz-summary/${course._id}`} key={index}>
              <div className="bg-indigo-200 p-4 rounded-xl h-72 shadow-md hover:shadow-lg transition duration-300 ease-in-out transform hover:-translate-y-1">
                ...
                />
                <h3 className="text-lg text-indigo-700 font-semibold mb-2">{course.courseTitle}</h3>
              </div>
            </Link>
          ))
        ) : (
          <div className="col-span-full text-center text-gray-500 text-lg">No quizzes found in your courses.</div>
        )}
      </div>
    </div>
  );
